...
class MLBacktester():
    ...
    def __init__(self, symbol, start, end, tc):
        ...
        self.model = xgb.XGBClassifier(
            ...
            num_class=3                 # Added to specify we have 3 classes (0, 1, 2)
        )
        self.results = None
    ...
    def fit_model(self, start, end):
        ...
        # Convert target classes from [-1, 0, 1] to [0, 1, 2] for XGBoost
        # XGBoost expects classes to be 0, 1, 2, etc. but np.sign returns -1, 0, 1
        self.y_original = np.sign(self.data_subset["returns"])
        self.y_transformed = self.y_original.copy()
        self.y_transformed = self.y_transformed.replace({-1: 0, 0: 1, 1: 2})
        
        self.model.fit(self.data_subset[self.feature_columns], self.y_transformed)
    ...
    def test_strategy(self, train_ratio = 0.7, lags = 5):
        ...
        raw_predictions = self.model.predict(self.data_subset[self.feature_columns])
        
        # Convert predictions back from [0, 1, 2] to [-1, 0, 1]
        prediction_map = {0: -1, 1: 0, 2: 1}
        predict = np.array([prediction_map[p] for p in raw_predictions])
    ...
    def plot_feature_importance(self):
        ...
        if hasattr(self, 'model') and hasattr(self.model, 'feature_importances_'):
            ...
            # If feature_importances_ is not available, return None
            return None, None
...
def main():
    ...
    The model predicts three possible states for price movement:
    - Upward movement (positive return)
    - No significant movement (near-zero return) 
    - Downward movement (negative return)
    """)
                ...
                with st.spinner("Running backtest..."):
                    try:
                        ...
                        with tab3:
                            ...
                            if feature_imp_fig is not None:
                                ...
                                st.warning("Feature importance information is not available for this model configuration.")
                                
                                # Information about model interpretation
                                st.info("""
                                **Note about multi-class XGBoost models:** 
                                Some XGBoost configurations may not provide direct feature importances.
                                This happens with certain multi-class setups.
                                """)
                        ...
                        if "Invalid classes" in error_msg:
                            st.info("""
                            **Troubleshooting:**
                            This error often happens when the target classes don't match XGBoost's expectations.
                            The app has been updated to handle this case by transforming the classes.
                            
                            If you're still seeing this error, please try:
                            1. Ensure your data has sufficient price movements (both up and down)
                            2. Try different symbols with more volatility
                            """)
                        elif "memory" in error_msg.lower():
                            st.info("""
                            **Troubleshooting:**
                            This appears to be a memory-related error. Please try:
                            1. Reduce the date range
                            2. Use fewer lags
                            3. Select a smaller subset of the data
                            """)
                        elif "cannot reshape" in error_msg.lower() or "dimension" in error_msg.lower():
                            st.info("""
                            **Troubleshooting:**
                            This appears to be a shape mismatch error. Please ensure:
                            1. Your data has consistent time intervals
                            2. There are no large gaps in your time series
                            """)
                        else:
                            st.info("""
                            **Troubleshooting:**
                            If you're encountering persistent errors, try:
                            1. Use the "Generate Sample Data" button to test with synthetic data
                            2. Ensure your CSV file has the correct format (see Data Preview tab)
