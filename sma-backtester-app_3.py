...
def test_sma_strategy(data, sma_s, sma_l):
    ...
    # Calculate cumulative returns - convert to float to avoid precision issues
    df_strat["creturns"] = df_strat["returns"].astype(float).cumsum().apply(np.exp)
    df_strat["cstrategy"] = df_strat["strategy"].astype(float).cumsum().apply(np.exp)
...
if df is not None:
    if strategy_type == "Data Overview":
        ...
        # Calculate drawdown - using a safer method to avoid division errors
        df_bh["cummax"] = df_bh.creturns.cummax()
        # Ensure numeric types and handle potential division issues
        df_bh["drawdown"] = (df_bh.creturns / df_bh.cummax.replace(0, np.nan)).sub(1).fillna(0)
