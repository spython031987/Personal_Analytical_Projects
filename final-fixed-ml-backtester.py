    ...
    st.warning("Please upload data or use the sample data to proceed with the analysis.")lags_min = st.number_input("Min Lags", min_value=1, max_value=10, value=1, step=1)
...
if data_option == "Use sample data":
        ...
        if st.button("Run Optimization"):
            ...
            st.info(f"💡 Tip: You can use these optimal parameters (Window: {int(best_params['Window'])}, TC: {best_params['TC']*100:.2f}%) in the Contrarian Strategy section.")try:
        ...
        if sma_s_max >= sma_l_min:
            ...
            if st.button("Run Optimization"):
                ...
                st.info(f"💡 Tip: You can use these optimal parameters ({int(best_params['SMA_S'])}, {int(best_params['SMA_L'])}) in the SMA Crossover strategy section.")# Create a price series with some trend and randomness
