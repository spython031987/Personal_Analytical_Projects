    ...
    st.warning("Please upload data or use the sample data to proceed with the analysis.")    elif strategy_type == "Cross-Pair Analysis":
        ...
        if st.button("Run Optimization"):
            ...
            st.info(f"💡 Tip: You can use these optimal parameters (Lags: {int(best_params['Lags'])}, TC: {best_params['TC']*100:.2f}%) in the ML Strategy section.")import streamlit as st
import pandas as pd
import numpy as np
...
if df is not None and selected_pair is not None:
    ...
    if strategy_type == "Data Overview":
        ...
        try:
            ...
            st.info("Try using different parameters or check if the data format is correct.")metric("Outperformance", metrics["Outperformance"])
