...
def test_sma_strategy(data, sma_s, sma_l):
    ...
    df_strat["creturns"] = df_strat["returns"].cumsum().apply(np.exp)
    df_strat["cstrategy"] = df_strat["strategy"].cumsum().apply(np.exp)
...
def calculate_metrics(returns_series, strategy_series=None):
    ...
    metrics["Sharpe Ratio"] = f"{(returns_series.mean() / returns_series.std()) * np.sqrt(252):.2f}"
    ...
    if strategy_series is not None:
        ...
        metrics["Strategy Sharpe Ratio"] = f"{(strategy_series.mean() / strategy_series.std()) * np.sqrt(252):.2f}"
...
if df is not None:
    if strategy_type == "Data Overview":
        ...
        # Calculate cumulative returns
        df_bh["creturns"] = df_bh.returns.cumsum().apply(np.exp)
        
        # Calculate drawdown
        df_bh["cummax"] = df_bh.creturns.cummax()
        df_bh["drawdown"] = df_bh.creturns / df_bh.cummax - 1
            ...
            if st.button("Run Optimization"):
                ...
                for i, (s_sma, l_sma) in enumerate(combinations):
                    ...
                    results.append({
                        ...
                        "annualized_return": df_test["strategy"].mean() * 252,
                        "annualized_risk": df_test["strategy"].std() * np.sqrt(252),
                        "sharpe": (df_test["strategy"].mean() / df_test["strategy"].std()) * np.sqrt(252)
                    })
                ...
                heatmap = ax.pcolor(pivot_data, cmap="viridis")
                ...
                ax.set_title("SMA Parameter Optimization Heatmap")
                ax.set_xlabel("Long SMA (SMA_L)")
                ax.set_ylabel("Short SMA (SMA_S)")
                
                # Add colorbar
                fig.colorbar(heatmap, ax=ax, label="Performance")
